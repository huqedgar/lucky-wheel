      ...
      wheelRef.current
        ...
        .on("end", async () => {
          ...
          if (isHostMode) {
            try {
              await Promise.all([
                supabase.from("participants").delete().eq("id", selectedParticipant.id),
  ...
  const renderWheel = useCallback(() => {
    ...
    svg
      ...
      .attr(
        "transform",
        `translate(${
          (wheelDimensions.width + WHEEL_CONSTANTS.PADDING.left + WHEEL_CONSTANTS.PADDING.right) /
            2 -
          15
        }, 12)`,
      )
      ...
      <div className="flex w-full flex-col gap-4 p-4 pt-6 md:w-1/2 md:p-8 md:pt-4">
        ...
        <div className="flex flex-col gap-4">
          ...
          <Textarea
            ...
            disabled={!isHostMode}
          />
          <Button onClick={handleParticipantInput} disabled={!isHostMode} className="w-full">
            Cập nhật danh sách
          </Button>
          {isHostMode && (
            <Button className="w-full" variant="destructive" onClick={handleClearAll}>
              Xóa tất cả
            </Button>
          )}
        </div>
        ...
        <div className="rounded-lg border border-gray-200 p-4">
          ...
          <div className="text-sm">Số người còn lại: {availableParticipants.length}</div>
        </div>
